...
describe("ParamUtils", () => {
    ...
    test("toTextOptionsNative", () => {
        const value = toTextOptionsNative({color: "#663212", position: {x: 50, y: 10}, text: "Bee", textSize: 32, thickness: 0});
    ...
    test("toBatchNative", () => {
        let value = toBatchNative({ operation: "text", options: {color: "#a3426f", position: {x: 12, y: 27}, text: "TEXT", textSize: 22, thickness: 1} },);
        expect(value).toEqual({
            operation: "text",
            options: {
        ...
        value = toBatchNative({ operation: "overlay", overlay: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/1000px-React-icon.svg.png", position: { x: 22, y: 73 } });
        expect(value).toEqual({
            operation: "overlay",
