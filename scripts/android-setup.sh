...
function createAVD {
  ...
  echo no | avdmanager create avd --name "$AVD_NAME" --force -k "$AVD_PACKAGES" -g google_apis -d "Nexus 4" --package "$AVD_PACKAGES"
  mkdir -p ~/.android/avd/$AVD_NAME.avd/
  cp $ANDROID_HOME/.android/avd/$AVD_NAME.avd/config.ini ~/.android/avd/$AVD_NAME.avd/
}

function launchAVD {
  # The AVD name here should match the one created in createAVD
  "$ANDROID_HOME/emulator/emulator" -avd "$AVD_NAME" -version
  if [ "$CI" ]
  then
    "$ANDROID_HOME/emulator/emulator" -verbose -avd "$AVD_NAME" -skin 470x860 -cores 1 -gpu auto -no-accel -memory 1024 -no-audio -no-snapshot -no-boot-anim -no-window
  else
